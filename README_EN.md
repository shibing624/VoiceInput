...
| 🌍 **Apple Speech multilingual mode** | Uses Apple Speech with switchable English, Simplified Chinese, Traditional Chinese, Japanese, and Korean |
| 📝 **Recording input box** | Open a standalone text box from the menu bar, collect ASR text, and copy it anywhere |
...
| **Speech Recognition** | Apple Speech multilingual recognition | System Settings › Privacy & Security › Speech Recognition |
...
You can switch the Apple Speech recognition language directly from the menu bar:

1. Open the menu bar icon
2. Go to **Language**
3. Choose English, Simplified Chinese, Traditional Chinese, Japanese, or Korean

### Recording Input Box

When you do not have a focused text field, or just want to collect transcription first:

1. Open the menu bar icon
2. Click **Recording Input Box...**
3. Press **Start Recording** in the panel
4. Speak, then press **Stop Recording**
5. Use **Copy** to copy the text, or keep editing it inside the panel
...
│   ├── UI/
│   │   ├── StatusBarController.swift  # Menu bar icon, recording state, feature entry points
│   │   ├── FloatingPanel.swift        # HUD capsule window
│   │   ├── RecordingInputPanel.swift  # Standalone recording text panel
...
│   ├── apple_speech_demo.py        # Standalone Apple Speech Python demo
│   └── build_apple_speech_demo_app.sh # Packages the Apple Speech demo as a .app
├── Info.plist
├── Entitlements.plist
├── Makefile
└── Package.swift
...
[Hold Fn / Right ⌘ / Click menu]
  ...
  CGEvent tap (FnKeyMonitor)
        │
        ├─► AVAudioEngine starts → microphone audio flows into Apple Speech
        │
        ├─► Apple Speech emits live partial / final text
        │
        ├─► Keyboard trigger: shown in the FloatingPanel HUD
        │
        └─► Menu trigger: shown in the Recording Input Box panel
        │
[Release key / Click "Stop Recording"]
  ...
  Final transcription text
        │
        ├─ (keyboard trigger + LLM enabled?) ──► OpenAI-compatible API ──► refined text
        │
        ├─► Keyboard trigger: TextInjector pastes into the current cursor position
        │
        └─► Menu trigger: text stays in the Recording Input Box for copy/edit
```

**Q: I want English or Japanese recognition instead of Chinese. What should I do?**  
A: Open the menu bar and choose the target language directly from **Language**.

**Q: What if there is no focused input box and I just want the transcription text first?**  
A: Open **Recording Input Box...** from the menu bar. The transcript stays in that panel so you can copy or edit it before pasting anywhere else.
